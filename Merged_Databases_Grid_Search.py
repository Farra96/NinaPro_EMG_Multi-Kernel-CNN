...
os.chdir('/home/riccardo/')
sys.path.append('./MKCNN')
...
# df_DB2 = pd.read_csv(f'/home/riccardo/EMG_data/DB2/Dataframe/dataframe_wnd_{wnd_len}.csv',
#                      dtype={'sub': np.int8, 'label': np.int8, 'rep': np.int8, 'sample_index': np.uint32})

# # DB7
# df_DB7 = pd.read_csv(f'/home/riccardo/EMG_data/DB7/Dataframe/dataframe_wnd_{wnd_len}.csv',
#                      dtype={'sub': np.int8, 'label': np.int8, 'rep': np.int8, 'sample_index': np.uint32})
...
# df_DB3 = pd.read_csv(f'/home/riccardo/EMG_data/DB3/Dataframe/dataframe_wnd_{wnd_len}.csv',
#                      dtype={'sub': np.int8, 'label': np.int8, 'rep': np.int8, 'sample_index': np.uint32})
...
# # Save dataframe
# df.to_csv(path_or_buf=f'/home/riccardo/EMG_data/DB2+DB7+DB3/Dataframe/dataframe_wnd_{wnd_len}.csv')
...
if __name__ == '__main__':
    ...
    # %% Load dataframe
    # Choose which window to use (so which dataframe to load)
    database = '/home/riccardo/EMG_data/DB2+DB7+DB3/'
    ...
    grid = {'net': {'N_multik': [32], 'N_Conv_conc': [128], 'N_SepConv': [128, 256],
                    'Kernel_multi_dim': [kernel_sizes[0], kernel_sizes[1], kernel_sizes[2]],
                    'Kernel_Conv_conc': [1],
                    'act_func': [nn.ReLU(), nn.LeakyReLU(), nn.ELU(), nn.PReLU()],
                    'Pool_Type': [nn.MaxPool2d, nn.AvgPool2d], 'wnd_len': [WND_LEN]
                    },
            'learning': {'n_models': [15],
                         'num_epochs': [100],
                         'lr': [0.001, 0.0006], # loguniform.rvs(1e-4, 1e-2, size=10),
                         'batch_size': [256, 512],
...
random_GS(grid=grid, train_set=train_set, valid_set=valid_set, test_set=test_set, cm_labels=exe_labels,
          path_to_save=f'/home/riccardo/EMG_data/DB2+DB7+DB3/grid_search_{WND_LEN}_{NORM_TYPE}_{RECT}')
